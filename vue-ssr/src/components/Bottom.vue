<template>
	<div class="bottom">
		<div v-for="(item, index) in bottomData" :key="item.path+index" @click="click(item)" :class="{bottomDiv: true, active: index === activIndex}">
			<i :class="item.class"></i>
			<p>{{item.text}}</p>
		</div>
		<transition name="slide-fade">
			<div class="consult" v-if="openPhoneType">
...
<script type="text/javascript">
	export default {
		...
		computed: {
			activIndex () {
				let that = this
				let idx = this.bottomData.findIndex(function (v) { return v.path === that.$route.name })
				return idx !== -1 ? idx : 0
			}
		},
		methods: {
